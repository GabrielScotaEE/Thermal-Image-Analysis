...
class imgProcessor():
    ...
    def __init__(self, base_colorbarScale=19.72, maximum_colorbarScale=25, sizeCrop_colorbar=111,
                 current_list=None, voltage_list=None):
    ...
    def show_images(self, rgb_crop_img, rgb_original_img, count, maxtemp, crop=False, analysis=None):

        fig, _ = plt.subplots(nrows=1, ncols=2)
        if analysis == '0':
            fig.canvas.manager.set_window_title('{}A. The Max Temperature is: {}'.format(self.current_list[count], maxtemp))
        elif analysis == '1':
